...
def main():
    parser = argparse.ArgumentParser(
        description="Convert MS MARCO TSV data to JSONL format."
    )
    parser.add_argument(
        "--collection_path", required=True, help="Path to the collection.tsv file."
    )
    parser.add_argument(
        "--queries_path", required=True, help="Path to the queries.train.tsv file."
    )
    parser.add_argument(
        "--out_dir", required=True, help="Directory to save the output JSONL files."
    )
    ...
    queries_df = pd.read_csv(
        args.queries_path, sep="\t", header=None, names=["qid", "text"]
    )
    ...
    with open(out_queries_path, "w") as f:
        for _, row in tqdm(
            ...
            desc="Converting queries.tsv to queries.jsonl",
        ):
