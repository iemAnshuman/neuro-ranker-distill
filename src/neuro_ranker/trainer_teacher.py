...
class TeacherTrainer:
    ...
    def fit(
        ...
        for ep in range(start_epoch, epochs):
            prog = tqdm(
                dl,
                desc=f"teacher ep{ep + 1}",
                initial=global_step % len(dl),
                total=len(dl),
            )
            run = 0.0
            for i, batch_x in enumerate(prog):
                ...
                out = self.model(
                    **{k: batch_x[k] for k in ["input_ids", "attention_mask", "labels"]}
                )
                loss = out.loss
                opt.zero_grad()
                loss.backward()
            ...
            if avg < best_loss:
                ...
                torch.save(
                    {"model": self.model.state_dict(), "name": self.model.name_or_path},
                    best_path,
                )
